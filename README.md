...
- Selected files are listed before upload (add more, remove any); each uploads in its own request with progress and automatic retry on dropped connections. Failed files stay in the list.
- The Uploaded table tags each file Ready or Done for the selected output format, and Convert shows how many are Ready.
