...
find "$UPLOADED_DIR" -maxdepth 1 -type f \( -iname '*.mp3' \) | sort | while IFS= read -r source_file; do
  ...
  if ! cp -- "$source_file" "$dest_old"; then
    echo "Failed copy to old: $file_name" >&2
    continue
  fi
  echo "Copied to old: $file_name"

  if ffmpeg -hide_banner -loglevel error -i "$dest_old" -map 0:a -c:a copy -write_xing 1 "$dest_converted"; then
